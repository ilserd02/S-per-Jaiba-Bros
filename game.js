...
class TitleScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image('brick', 'assets/blocks/overworld/brick.png'); // Asegúrate de tener este asset para los bloques normales
...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Distribución basada en montañas y nubes de la captura original
    ...
    // --- DISTRIBUCIÓN EXACTA DEL NIVEL 1-1 ---

    // 1. Primer Bloque de Misterio Solitario (Contiene un Hongo)
    this.createMysteryBox(256, config.height - 80, true);

    // 2. Primera Estructura Combinada (Ladrillo - Misterio - Ladrillo - Misterio - Ladrillo)
    this.createBrick(320, config.height - 80);
    this.createMysteryBox(336, config.height - 80, false); // Moneda/Vacío
    this.createBrick(352, config.height - 80);
    this.createMysteryBox(368, config.height - 80, false); 
    this.createBrick(384, config.height - 80);

    // 3. Bloque de Misterio Elevado superior
    this.createMysteryBox(352, config.height - 130, false);
    ...
    // Reproducir brillo en todos los bloques sorpresa creados
    ...
    // Registrar animaciones de Mario/Jaiba de forma segura
    this.registerPlayerAnimations();

    // Creación del jugador (Jaiba)
  ...
  // Métodos auxiliares de creación limpia
  ...
  createBrick(x, y) {
    // Si no tienes una textura llamada 'brick', Phaser usará un cuadro verde por defecto de manera segura
  ...
  createGoomba(x, y) {
    ...
    // Hace que cambie de dirección de manera clásica al chocar con las tuberías
  ...
  update() {
    this.goombas.children.iterate(goomba => {
      if (goomba && goomba.body && goomba.body.enable && this.anims.exists('goomba-walk')) {
        ...
        // Voltea el sprite dependiendo de hacia dónde camina
